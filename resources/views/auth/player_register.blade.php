@extends('layouts.app')
@section('player_register')

<body>
    <div class="text-center">
        <img src="logo_header.png" alt="サンプル画像" style="width:350px;">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-6">
                <h1 class="display-1 font-left-player">ユーザー<br>情報入力</h1>
            </div>
            <div class="col-md-6 mt-5">
                <form>
                    <fieldset>
                        <legend class="font-right-player">以下の必要情報を入力してください。</legend>
                        <div class="mb-3 font-right-player">
                            <label for="sportSelect" class="form-label">競技名</label>
                            <select id="sportSelect" class="form-select">
                                <option value="soccer">サッカー</option>
                                <option value="baseball">野球</option>
                                <option value="basketball">バスケットボール</option>
                            </select>
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="firstNameInput" class="form-label">姓</label>
                                    <input type="text" id="firstNameInput" class="form-control" placeholder="姓">
                                </div>
                                <div class="col-md-6">
                                    <label for="lastNameInput" class="form-label">名</label>
                                    <input type="text" id="lastNameInput" class="form-control" placeholder="名">
                                </div>
                            </div>
                            <label for="genderSelect" class="form-label">性別</label>
                            <select id="genderelect" class="form-select">
                                <option value="male">男性</option>
                                <option value="female">女性</option>
                            </select>
                            <label for="birth" class="form-label">生年月日</label>
                            <input type="date" id="birth" class="form-control">
                        </div>
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="disabledFieldsetCheck" disabled>
                                <label class="form-check-label" for="disabledFieldsetCheck">
                                    Can't check this
                                </label>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary">Submit</button>
                    </fieldset>
                </form>
            </div>
        </div>

</body>
@endsection